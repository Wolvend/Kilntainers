...
class TestE2EGracefulShutdown:
    ...
    @pytest.mark.docker_integration
    @pytest.mark.asyncio
    async def test_graceful_shutdown_on_stdin_eof(self, docker_available):
        """Server exits cleanly and removes container when stdin closes."""

        async def run_session():
            # Get initial container count
            result = subprocess.run(
                ["docker", "ps", "-q", "--filter", "label=kilntainers=true"],
                capture_output=True,
                text=True,
            )
            initial_count = (
            ...
            assert final_count == initial_count, "Container was not cleaned up"

            # Read and discard stderr
            _ = await proc.stderr.read()
